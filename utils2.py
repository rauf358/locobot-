...
def load_known_faces(face_data_path, names_path):
    ...
    try:
        ...
        if loaded_embeddings_np.size > 0:
            ...
            norms = np.linalg.norm(embeddings_for_faiss, axis=1, keepdims=True)
            norms[norms == 0] = 1e-10
            normalized_embeddings = embeddings_for_faiss / norms

            faiss_idx.add(normalized_embeddings)
            faiss_idx_map = list(range(len(loaded_names)))
            print(f"FAISS index created and populated with {faiss_idx.ntotal} vectors (normalized for cosine similarity).")
        else:
